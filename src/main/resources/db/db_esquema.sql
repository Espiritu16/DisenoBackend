...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_caso_resuelto_set_fecha` BEFORE UPDATE ON `caso_operativo` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_historial_caso_on_update` AFTER UPDATE ON `caso_operativo` FOR EACH ROW BEGIN
  IF OLD.estado <> NEW.estado THEN
    INSERT INTO historial_estado_caso (
      ...
      OLD.estado,
      NEW.estado,
      'Cambio de estado automatico desde actualizacion de caso',
      NEW.id_responsable,
      CURRENT_TIMESTAMP
    );

    -- Sincroniza estado del reporte origen con el caso
    UPDATE reporte
    SET estado = CASE
      WHEN NEW.estado = 'EN_PROCESO' THEN 'EN_PROCESO'
      WHEN NEW.estado = 'RESUELTO' THEN 'RESUELTO'
      WHEN NEW.estado = 'ESCALADO' THEN 'ESCALADO'
      WHEN NEW.estado = 'RECHAZADO' THEN 'RECHAZADO'
      ELSE estado
    END,
    fecha_actualizacion = CURRENT_TIMESTAMP
    WHERE id_reporte = NEW.id_reporte_origen;
  END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_historial_reporte_on_update` AFTER UPDATE ON `reporte` FOR EACH ROW BEGIN
  IF OLD.estado <> NEW.estado THEN
    INSERT INTO historial_estado_reporte (
      ...
      OLD.estado,
      NEW.estado,
      'Cambio de estado automatico desde actualizacion de reporte',
      NEW.id_usuario,
      CURRENT_TIMESTAMP
    );
  END IF;
